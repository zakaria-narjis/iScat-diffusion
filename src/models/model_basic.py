...
class conv_block(nn.Module):
    """Conv block with GroupNorm instead of BatchNorm"""
    def __init__(self, ch_in, ch_out, temb_dim, num_groups=8):
        super().__init__()
        # GroupNorm: num_groups default 8
...
class U_Net(nn.Module):
    def __init__(self, img_ch=3, cond_ch=1, output_ch=1, temb_dim=256, num_groups=8):
        ...
        # encoder conv blocks
        self.Conv1 = conv_block(img_ch + cond_ch, 64, temb_dim, num_groups)
        self.Conv2 = conv_block(64, 128, temb_dim, num_groups)
        self.Conv3 = conv_block(128, 256, temb_dim, num_groups)
        self.Conv4 = conv_block(256, 512, temb_dim, num_groups)
        self.Conv5 = conv_block(512, 1024, temb_dim, num_groups)

        # decoder conv blocks
        self.Up5 = up_conv(1024, 512, num_groups)
        self.Up_conv5 = conv_block(1024, 512, temb_dim, num_groups)
        self.Up4 = up_conv(512, 256, num_groups)
        self.Up_conv4 = conv_block(512, 256, temb_dim, num_groups)
        self.Up3 = up_conv(256, 128, num_groups)
        self.Up_conv3 = conv_block(256, 128, temb_dim, num_groups)
        self.Up2 = up_conv(128, 64, num_groups)
        self.Up_conv2 = conv_block(128, 64, temb_dim, num_groups)
